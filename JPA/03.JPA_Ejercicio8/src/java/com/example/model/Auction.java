...
@NamedQueries(
        {
            ...
            @NamedQuery(name = "Auction.findBySeller", query = "SELECT a FROM Auction a WHERE a.seller = :seller"),
            @NamedQuery(name = "Auction.findByStatus", query = "SELECT a FROM Auction a WHERE a.status = :status"),
            @NamedQuery(name = "Auction.findByStatusByCondition", query = "SELECT a FROM Auction a WHERE a.status = :status "
                    + "AND a.item.condition = :condition "),
            @NamedQuery(name = "Auction.findBySellerByStatus", query = "SELECT a FROM Auction a WHERE a.seller = :seller "
                    + "AND a.status = :status"),
            @NamedQuery(name = "Auction.findBySellerByStatusByCondition", query = "SELECT a FROM Auction a WHERE a.seller = :seller "
                    + "AND a.status = :status "
                    + "AND a.item.condition = :condition")
        }
)
public class Auction
    ...
    private static final long serialVersionUID = -5299569462269081869L;
    ...
    public Auction(int auctionId, AuctionUser seller, Item item,
            float currPrice, float increment, AuctionStatus status, Date endDate) {
