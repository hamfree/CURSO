...
public class AuctionFacade
{
        ...
        ItemCondition condition )
    {
        ...
        try {
            ...
            Predicate criteria = cb.equal( auction.get( Auction_.status ), paramStatus ); // a.status = :status

            if ( condition != null ) {
                ParameterExpression<ItemCondition> paramCond
                    = cb.parameter( ItemCondition.class, "condition" );  // :condition
                Expression<ItemCondition> path
                    = auction.get( Auction_.item ).get( Item_.condition );
        ...
        AuctionStatus status, ItemCondition condition )
    {
        ...
        try {
            ...
            Predicate criteria = cb.equal( auction.get( Auction_.seller ), paramUser ); // a.seller = :seller
            ...
            if ( condition != null ) {
                ...
                Expression<Item> path = auction.<Auction>get( "item" ).<Item>get( "condition" );
                criteria = cb.and( criteria, cb.equal( path, paramCond ) );  // AND a.item.condition = :condition
            }
            cq.select( auction ).where( criteria );
            cq.orderBy( cb.asc( auction.get( Auction_.endDate ) ), cb.asc( auction.get( Auction_.currPrice ) ) );  // ORDER BY a.endDate ASC, a.currPrice ASC
