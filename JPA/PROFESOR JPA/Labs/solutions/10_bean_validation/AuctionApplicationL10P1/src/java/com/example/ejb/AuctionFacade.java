...
public class AuctionFacade
{
    ...
    public AuctionListView[] getSearchListView( String searchStr )
    {
        ...
        Expression<String> path = auction.get( "item" ).get( "title" ); // a.item.title
    ...
    private AuctionUser getUserByName( String userName )
    {
        ...
        if ( userName != null ) {
            ...
            cq.select( auctionUser ).where( cb.equal( auctionUser.get( "displayName" ), userName ) );  // SELECT au FROM AuctionUser au WHERE au.displayName = username
    ...
    public AuctionListView[] getAllAuctionListView()
    {
        ...
        try {
            ...
            cq.orderBy( cb.asc( auction.get( "endDate" ) ), cb.asc( auction.get( "currPrice" ) ) );  // ORDER BY a.endDate ASC, a.currPrice ASC
    ...
    public AuctionListView[] getAllAuctionListView( AuctionStatus status,
        ItemCondition condition )
    {
        ...
        try {
            ...
            criteria = cb.equal( auction.get( "status" ), paramStatus ); // a.status = :status

            if ( condition != null ) {
                ParameterExpression<ItemCondition> paramCond = cb.parameter( ItemCondition.class, "condition" );  // :condition
                Expression<Item> path = auction.get( "item" ).get( "condition" );
    ...
    public AuctionListView[] getAllAuctionListView( String userName,
        AuctionStatus status, ItemCondition condition )
    {
        Auction[] auctionList = null;
        TypedQuery<Auction> auctionQuery;
        try {
            ...
            criteria = cb.equal( auction.get( "seller" ), paramUser ); // a.seller = :seller

            ParameterExpression<AuctionStatus> paramStatus = cb.parameter( AuctionStatus.class, "status" );  // :status
            criteria = cb.and( criteria, cb.equal( auction.get( "status" ), paramStatus ) ); // AND a.status = :status
